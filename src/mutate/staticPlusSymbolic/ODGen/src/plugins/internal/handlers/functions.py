...
def handle_require(G: Graph, caller_ast, extra, _, module_names):
    ...
    if G.exit_when_found and G.vul_type not in ['proto_pollution', 'ipt'] and G.vul_type != 'dataflow':
        vul_type = G.vul_type
...
def run_exported_functions(G, module_exports_objs, extra):
    ...
    while(len(exported_objs) != 0): # BFS
        ...
        if G.exit_when_found and vul_type not in ['proto_pollution', 'ipt'] and vul_type != 'dataflow':
            res_path = traceback(G, vul_type)
            res_path = vul_checking(G, res_path[0], vul_type)
            if len(res_path) != 0:
...
def call_function(G, func_objs, args=[], this=NodeHandleResult(), extra=None,
    ...
    '''
    if options.allow_func is not None:
        allow_func_list = options.allow_func.split(',')
        allow_func_list.append("anonymous")
    if func_name not in allow_func_list:
        print("Skipping "+func_name)
        return NodeHandleResult(), []
    '''
