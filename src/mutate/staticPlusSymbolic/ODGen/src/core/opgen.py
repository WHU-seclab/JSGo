...
from .checker import traceback, vul_checking, traceback_dataflow
...
class OPGen:
    ...
    def check_dataflow(self, G):
        # We do backward dataflow analysis
        # There is no need to call vul_checking() 
        pathes = traceback_dataflow(G)[0]
    ...
    def _test_graph(self, G: Graph, vul_type='os_command'):
        ...
        G.export_to_CSV("./exports/nodes.csv", "./exports/rels.csv", light=False)

        print("Finishing generating ODGraph")

        if options.gen:
            print("Stop after ODGraph is generated")
            return []
    ...
    def run(self):
        ...
        if options.dataflow:
            if not os.path.exists("/ODGen/ODG.csv"):
                 loggers.error_logger.error("[ERROR] ODGraph '/ODGen/ODG.csv' not found")
            self.run_dataflow(self.graph)
            return
